//! Archives a session transcript into a bank's `sessions/` tier; `src` is only removed after the compressed copy verifies byte-identical.
...
/// Gzip-compress `src` to `dest_gz`, verify it decompresses back byte-identical, then remove `src`; left untouched on any failure.
...
mod tests {
    ...
    fn source_is_left_in_place_when_the_destination_cannot_be_written() {
        ...
        // A parent that's a file, not a directory, forces create_dir_all to fail.
